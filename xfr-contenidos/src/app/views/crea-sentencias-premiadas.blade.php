<?php
function get_view_sentencias_premiadas_create($xfrContenidos) {
    ...
    cursor: pointer;
		...
		<div id="wrap_contenidos" class="bg-content frctl">
				<div class=" bg-white col-xs-12 col-sm-offset-1_ col-sm-12  col-md-offset-1_ col-md-11 col-lg-offset-1 col-lg-10 col-xl-offset-2 col-xl-8  ">
					<div class="pl40 mb10">
						<h2 class="fw600 text-555 ">Gestión de Sentencias Premiadas</h2>
					</div>
...
<script>
  jQuery(function ($) {     
    ...
    let regmodel = {
        ...
        sections: [
          {
            ...
            fields: [
              { field: 'id_sentencia', type: 'hidden', },
              {
              ...
              // {
              //   field: 'fechayhora_enviado', type: 'datetime-local', label: 'Fecha y hora en que se envió', placeholder: '', title: '', help: '',
              ...
              {
                field: 'anio', type: 'number', label: 'Año', placeholder: '', title: '', help: '',
                required: false, columns_4: 2, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'idp_sentencia_materia', type: 'select', label: 'Materia', placeholder: '', title: '', help: '',
                required: true, columns_4: 2, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'idp_sentencia_tipo', type: 'select', label: 'Tipo de Resolución', placeholder: '', title: '', help: '',
                required: false, columns_4: 2, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'descriptores', type: 'text', label: 'Descriptores', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'dictada', type: 'text', label: 'Dictada por:', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'autoridades', type: 'text', label: 'Autoridades', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'hechos', type: 'textarea', label: 'Hechos', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'proceso1', type: 'textarea', label: 'Proceso1 (Identificación del problema jurídico y la def. de persona ... )', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'proceso2', type: 'textarea', label: 'Proceso2 (Det. del derecho aplicable y prob... )', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'proceso3', type: 'textarea', label: 'Proceso3 (Det. de los hechos y valoración de la prueba... )', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'proceso4', type: 'textarea', label: 'Proceso4 (Parte resolutiva y reparación del daño )', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'analisis', type: 'textarea', label: 'Análisis de Género', placeholder: '', title: '', help: '',
                required: false, columns_4: 4, class: { bloque: '', group: 'has-primary_', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
      ...
      listasPredefinidas: {
        estados: [{ key: 'nueva', texto: 'Nueva' }, { key: 'relevante', texto: 'Relevante' }, 
        { key: 'premiada', texto: 'Premiada' }, { key: 'eliminada', texto: 'Eliminada' }],
      },
      ...
      create_fields: (sections) => {
        _.forEach(sections, (sec) => {
          ...
          _.forEach(sec.fields, (field) => {
            ...
            if (field.type == 'text' || field.type == 'number' || field.type == 'date' || field.type == 'datetime-local' || field.type == 'password' || field.type == 'email') {
              let htmlField =
                  /*html*/`
      ...
      inicializaControles: () => {
        ...
        /*     SENTENCIAS_MATERIAS       */
        $.post(ctxG.rutagral + '/get-parametros-from', {dominio: 'sentencias_materias'}, res =>{
          let opts = xyzFuns.generaOpciones(res.data, 'id', 'descripcion', ' ');
          $("[__rg_field=idp_sentencia_materia]").html(opts).append(itemCrearNuevo);
        });
        /*     SENTENCIAS_TIPOS       */
        $.post(ctxG.rutagral + '/get-parametros-from', {dominio: 'sentencias_tipos'}, res =>{
          let opts = xyzFuns.generaOpciones(res.data, 'id', 'descripcion', ' ');
          $("[__rg_field=idp_sentencia_tipo]").html(opts).append(itemCrearNuevo);
        });
          ...
          columns: [
            ...
            { title: 'vacio', orderable: false, visible: false, render: () => { return '' } },
            {
              ...
              render: function (data, type, row, meta) {
                let html = /*html*/`
                      <div __accion="editar" __id_sentencia="${row.id_sentencia}" class="cursor ${row.estado_contenido == 'eliminada' ? 'text-danger-darker':  (row.estado_contenido == 'premiada' ? 'text-success-darker ' : 'text-system-darker ' )}"  >
                        <i class=" glyphicon glyphicon-book fa-lg"></i> <span>${row.estado_contenido}<i>
            ...
            {
              title: 'Titulo/Tema', data: 'titulo', orderable: true, searchable: true, className: 'mnw200 fw600', type: 'html', 
              render: function (data, type, row, meta) {
                let html = /*html*/`
                      <div __accion="editar" __id_sentencia="${row.id_sentencia}" class="cursor">
                        <!--<img src='${row.imagen_sm}' alt="" style="width:100%; max-width: 200px; min-width: 130px" >-->
                        <span class=" ">${row.titulo }</span>
                      </div>`;
                return html;
              }
            },
            ...
            {
              title: 'Materia', data: 'materia', orderable: true,
            },
            {
              title: 'Tipo', data: 'tipo', orderable: true,
            },
            {
              title: 'Descriptores', data: 'descriptores', orderable: true,
            },
            {
              title: 'Dictada por', data: 'dictada', orderable: true,
            },
            {
              title: 'Autoridades', data: 'autoridades', orderable: true,
            },
            {
              title: 'Orden', data: 'orden', orderable: true,
            },
          ],
    ...
    let funs = {
      ...
      nuevo: () => {
        $("#modal [__cabecera_modal] span").html(`Sentencias con Perspectiva de Género`);
      ...
      editar: (id) => {
        ...
        funs.spinner();
        funs.crearFormulario();
        $.post(ctxG.rutabase + '/get-content-sentencia', { id_sentencia: id_sentencia }, (resp) => {
          let data = resp.data;
      ...
      saveData: () => {
        ...
        $.ajax({
            url: ctxG.rutabase + '/save-sentencia-upload',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
      ...
      $(ctxG.content)
        ...
        .on('click', '[__accion]', (e) => {
          ...
          if (accion == 'nuevo')
            funs.nuevo();
          if (accion == 'editar') {
            let id = $(e.currentTarget).attr('__id_sentencia');
            funs.editar(id)
          }
        })
