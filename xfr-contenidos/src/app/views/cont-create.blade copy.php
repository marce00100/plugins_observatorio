...
<style>
	...
	.tabla-head thead>th {
		...
		<div id="wrap_contenidos" class="bg-content frctl">
			...
			<!-- -----------------------------------------          Modal  --------------------------------------------------- -->
			<div id="modal" class="white-popup-block popup-basic mfp-with-anim mfp-hide">
				<div class="panel frctl">
					<!-- panel heading -->
					<div class="bg-system darker text-333 flex align-center p15 ">
						<i class="fa fa-paperclip fa-lg ml10"></i>
						<h2 class="panel-title ml10" __titulo><span>__</span></h2>
					</div>
					...
					<div class="panel-body of-a">
						<div class="row">
							<div class=" ph40 ">
								<!-- datos de usuario -->
								<div __fields></div>
                <textarea name="" id="" __rg_field="texto"  cols="30" rows="10"></textarea>
								<hr>
              </div>
						</div>
					</div>
  ...
  jQuery(function ($) {     
    ...
    let regmodel = {
        ...
        sections: [
          {
            ...
            fields: [
              ...
              {
                field: 'imagen', type: 'file', label: 'Imagen', placeholder: '', title: '', help: '',
                required: true, grid_column_span: 2, class: { bloque: '', group: 'has-primary', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
              ...
              // {
              //   field: 'texto', type: 'text', label: 'Texto', placeholder: '', title: '', help: '',
              ...
              {
                field: 'fecha_publicacion', type: 'date', label: 'Fecha dePublicación', placeholder: '', title: '', help: '',
                required: false, grid_column_span: 1, class: { bloque: '', group: 'has-primary', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
                field: 'sub_tipo', type: 'select', label: 'Categoria', placeholder: '', title: '', help: '',
                required: false, grid_column_span: 1, class: { bloque: '', group: 'has-primary', label: 'form-label', icon: '', input: 'form-input p5 wp100' },
              },
              {
      ...
      listasPredefinidas: {
        ...
        sub_tipo: ['Nacionales','Internacionales', 'Institucionales'],
        tipo_contenido: ['Noticias','Actividades'],
      },
        ...
        _.forEach(sections, (sec) => {
          ...
          _.forEach(sec.fields, (field) => {
            ...
            if (field.type == 'text' || field.type == 'number' || field.type == 'date' || field.type == 'password' || field.type == 'email') {
              htmlFields +=
                  /*html*/`
                  <div class="form-horizontal_ ${field.class.bloque || ''} ${field.grid_column_span == 2 ? 'grid-column-span-2' : ''}" >
            ...
            if (field.type == 'select') {
              htmlFields +=
                  /*html*/`
                  <div class="form-horizontal_ ${field.grid_column_span == 2 ? 'grid-column-span-2' : ''} ">
                    <div class="form-group  ${field.class.group || ''} mbn ">
                        <label class="col-xs-11 ${field.grid_column_span == 2 ? 'col-sm-3' : 'col-sm-6'} control-label ${field.class.label}" for="${field.field}"
                        style="">${field.label}</label>
                        <div class="col-xs-11   ${field.grid_column_span == 2 ? 'col-sm-9' : 'col-sm-6'}">
            ...
            if (field.type == 'file') {
              htmlFields +=
                  /*html*/`
                  <div class="form-horizontal_ grid-column-span-2 ">
                    <div class="form-group  has-primary mbn ">
                        <label class="col-xs-11 col-sm-3 control-label form-label" for="imagen" style="">Imagen</label>
                        <input type="file" __archivo_up __imagen_guardada __imagen_nueva class="form-input p5 wp100 form-control pl10 col-xs-9" id="imagen" __rg_field="imagen" name="imagen" title="" required="">
      ...
      inicializaControles: () => {
        ...
        let sub_tipo = xyzFuns.generaOpcionesArray(regmodel.listasPredefinidas.sub_tipo);
        $("[__rg_field=sub_tipo]").html(sub_tipo);
        $("[__rg_field=sub_tipo] option")[0].selected = true;
        ...
        // $.post(xyzFuns.urlRestApiWP + 'gral/v1/get-parametros-from', {dominio: 'tipo_contenido'}, res =>{
        //   letoptsTipoContenido = xyzFuns.generaOpciones(res.data, 'nombre', 'descripcion');
        //   $("[__rg_field=tipo_contenido]").html(letoptsTipoContenido);
        // });
    ...
    let funs = {
      ...
      saveData: () => {
        ...
        funs.spinner();
        $.post(ctxG.rutabase + '/save-content', { data_save: obj }, function (resp) {
