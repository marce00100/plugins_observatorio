...
class ContenidosMigrateController extends MasterController {
  ...
  private function migra($sistema, $tipo_contenido){
    ...
    if ($sistema == 'observatorio' && 
      ...
      $config = (object)json_decode($param->config);

      if($config->migrate && $config->migrate == 1){
        return (object)[
          'status' => 'ok', 'msg' => 'Esta migracion ya se hizo anteriormente'
        ];
      }
      ...
      $DB->statement(" ALTER TABLE {$config->tabla} ADD COLUMN IF NOT EXISTS orden int ");
      $DB->statement(" UPDATE {$config->tabla} t set t.estado = 1");
      ...
      foreach ($list as $item) {
        $fieldsUpdate = (object)[];
        $imagenes = collect($DB->select("SELECT * from imagenes where codigo = {$item->$nombre_cod_biblioteca} and modulo = '{$nombre_modulo}' AND categoria LIKE '%grand%'"))->pluck('nombre')->implode(',');
        $fieldsUpdate->imagen = $imagenes;
        ...
        $fieldsUpdate->archivos = $archivos;
        global $wpdb;
      ...
      if($tipo_contenido == 'normas' || $tipo_contenido == 'jurisprudencia'){
        $DB->statement(" UPDATE {$config->tabla} t set t.categoria    = ( select p.id from xfr_parametros p where p.dominio = '{$paramCategoria}' and p.nombre = t.categoria)");
      }
